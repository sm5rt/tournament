...
class TournamentStates(StatesGroup):
    ...
    waiting_for_member1 = State()
    waiting_for_member2 = State()
    waiting_for_member3 = State()
    current_team_index = State()
    total_teams = State()
    teams_data = State()
...
async def cmd_help(message: Message):
    help_text = (
        ...
        "🔹 <b>3. Ввод команд</b>\n"
        "Бот будет запрашивать данные <b>по одной команде</b>:\n"
        "— Название\n— Игрок 1\n— Игрок 2\n— Игрок 3\n\n"
        "🔹 <b>4. Рандом-турнир</b>\n"
...
async def handle_format(callback: CallbackQuery, state: FSMContext):
    ...
    if fmt == "random":
        ...
        await callback.message.edit_text(
            f"Выбран формат: {fmt} команд.\nЧто делаем?",
            reply_markup=expand_or_start_kb(fmt)
        )
        await state.set_state(TournamentStates.choosing_format)
    await callback.answer()

@router.callback_query(F.data.startswith("expand_"))
async def expand_format(callback: CallbackQuery, state: FSMContext):
    new_fmt = callback.data.replace("expand_", "")
    await state.update_data(format=new_fmt)
    await callback.message.edit_text(
        f"Выбран формат: {new_fmt} команд.\nЧто делаем?",
        reply_markup=expand_or_start_kb(new_fmt)
    )
    await callback.answer()

@router.callback_query(F.data == "start_tournament")
async def confirm_start_tournament(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    fmt = data['format']
    await state.update_data(total_teams=int(fmt))
    await state.update_data(current_team_index=1)
    await state.update_data(teams_data=[])
    await callback.message.answer(f"📝 Введите название команды 1 из {fmt}:")
...
async def get_team_name(message: Message, state: FSMContext):
    await state.update_data(current_team_name=message.text.strip())
    await message.answer("👤 Введите первого участника:")
    await state.set_state(TournamentStates.waiting_for_member1)

@router.message(TournamentStates.waiting_for_member1)
async def get_member1(message: Message, state: FSMContext):
    await state.update_data(current_member1=message.text.strip())
    await message.answer("👤 Введите второго участника:")
    await state.set_state(TournamentStates.waiting_for_member2)

@router.message(TournamentStates.waiting_for_member2)
async def get_member2(message: Message, state: FSMContext):
    await state.update_data(current_member2=message.text.strip())
    await message.answer("👤 Введите третьего участника:")
    await state.set_state(TournamentStates.waiting_for_member3)

@router.message(TournamentStates.waiting_for_member3)
async def get_member3(message: Message, state: FSMContext):
    current_member3 = message.text.strip()
    data = await state.get_data()
    team = (data['current_team_name'], [data['current_member1'], data['current_member2'], current_member3])
    teams_data = data.get('teams_data', [])
    teams_data.append(team)
    await state.update_data(teams_data=teams_data)
    current_index = data['current_team_index']
    total = data['total_teams']
    if current_index < total:
        next_index = current_index + 1
        await state.update_data(current_team_index=next_index)
        await message.answer(f"📝 Введите название команды {next_index} из {total}:")
        await state.set_state(TournamentStates.waiting_for_team_name)
    else:
